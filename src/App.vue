...
let id = 0;
const guesses = ref([
  {id:id++, color:'#141414', text:'This is some text inside of a div block.'},
  {id:id++, color:'#141414', text:'This is some text inside of a div block.'},
  {id:id++, color:'#141414', text:'This is some text inside of a div block.'},
  {id:id++, color:'#141414', text:'This is some text inside of a div block.'},
  {id:id++, color:'#141414', text:'This is some text inside of a div block.'},
...
onMounted(async () => {
  try {
    ...
    if(!window.localStorage.getItem("selected_game")){
      ...
      gameSelectionList.value = JSON.parse(window.localStorage.getItem("selected_game"));
    }
...
onUnmounted(() => {
  window.URL.revokeObjectURL(currentTrack.value.file);
});

function initGameSelectionList() {
  let newGameSelectionList = [];
  games.value.forEach(franchise => {
      franchise.games.forEach(game => {
          newGameSelectionList.push(game.id);
      });
  });
  console.log("in GameSelection - newGameSelectionList", newGameSelectionList);
  gameSelectionList.value = newGameSelectionList;
}

function getGameSelectionPlaylists() {
  var result = userPlaylists.value.filter(playlist => {
    let idgameSelectionList = gameSelectionList.value.find(idGamePlaylist => idGamePlaylist == playlist.id);
    if(idgameSelectionList){
      return playlist;
    }
  })
  if (result.length == 0){
    result.push(userPlaylists.value[1]);
  }
  console.log('in app - Playlists après filtrage pour la sélection de jeu:', result);
  return result;
}

async function getRandomTrack(){
  var playlists = getGameSelectionPlaylists();
  const randomPlaylistIndex = Math.floor(Math.random() * playlists.length);
  console.log('in app - randomPlaylistIndex:', randomPlaylistIndex);
  var randomPlaylist = playlists[randomPlaylistIndex];
  console.log('in app - Playlist aléatoire sélectionnée:', randomPlaylist);
...
async function makeFilterGameList() {
  ...
  userPlaylists.value.forEach(playlist => {
    ...
    if (findFranchise) {
      findFranchise.games.push(game);
    } else {
      result.push({
        name: franchisePlaylist,
        games: [game]
      });
    }
  })
  console.log('in app - result filterGameList:', result);
  return result;

  // creer un objet avec la franchise et les jeux associés
  // objet franchise: {franchise: 'franchise1', games: [{franchise: 'franchise1', game: 'game1',id: id1}, {franchise: 'franchise1', game: 'game2', id: id2}, ...]},
  // ajouter l'objet à la liste result
}

function onSubmit() {
  answerStatus.value = checkAnswer(userGuess.value, currentTrack.value);
  //console.log('Is the user guess correct?', answerStatus.value);
  let currentGuess = guesses.value[guessNumber.value];
  currentGuess.text = userGuess.value.title + ' - ' + userGuess.value.game;
  userGuess.value = '';
  guessNumber.value++;
  console.log('in app - guess number:', guessNumber.value);

  switch (answerStatus.value) {
    case 'franchise':
      currentGuess.color = 'yellow';
      break;
    case 'game':
      currentGuess.color = 'green';
      break;
    case 'title':
      //message.value = '✅ Franchise correcte !';
      break;
    default:
      currentGuess.color = 'red';
  }

  if(guessNumber.value >= guesses.value.length){
    answerStatus.value = 'failed';
  }
}

function onSkip() {
  let currentGuess = guesses.value[guessNumber.value];
  currentGuess.text = 'Réponse passée';
  guessNumber.value++;
  if(guessNumber.value >= guesses.value.length){
    answerStatus.value = 'failed';
  }
...
/**
   ...
   * @returns {string} 'title', 'game' ou 'franchise' selon le niveau de correspondance
   * @example
   * const isCorrect = soundCloudClient.checkAnswer('Chill Lo-fi', 'chill lo-fi beats');
   */
function checkAnswer(userGuess, correctTitle) {
  console.log('in app - user guess:', userGuess);
  console.log('in app - normalized user guess:', normalize(userGuess));
  var normalizedUserGuess = normalize(userGuess);
  var normalizedCorrectTitle = normalize(correctTitle);

  if(normalizedUserGuess.title == normalizedCorrectTitle.title){
    return 'title';
  }

  else if(normalizedUserGuess.game == normalizedCorrectTitle.game){
    return 'game';
  }

  else if(normalizedUserGuess.franchise == normalizedCorrectTitle.franchise){
    return 'franchise';
  }
}
...
function normalize(item) {
  return {
    franchise: item.franchise.toLowerCase().trim(),
    game: item.game.toLowerCase().trim(),
    title: item.title.toLowerCase().trim()
  }
}

async function getUserTracksTitles(userId){
  var tracks = [];
  userPlaylists.value.forEach(playlist => {
...
function formatTrack(track){
  var trackTitle = titles.value.find(title => title.id === track.id);
  //console.log('in app - Track title found for current track:', trackTitle);
  currentTrackDetails.value.game = trackTitle.game;
  currentTrackDetails.value.title = trackTitle.title;
  console.log('in app - Track after formatting:', currentTrackDetails.value);
}
</script>
...
:root {
  ...
  --radiant-ui-components-library-marketplace--linear-color-three: #444;
...
.rt-component-section.rt-pogress-bar-v-one {
  padding-top: 20px;
  padding-bottom: 20px;
}
...
.rt-component-heading-five.rt-margin-top-ten {
  margin-top: 10px;
}
