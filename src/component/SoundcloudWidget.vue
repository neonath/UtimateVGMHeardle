<template>
    <a :href="props.track.permalink_url" target="_blank" rel="noopener noreferrer"
        :title="`Ecoutez ${props.track.title} sur SoundCloud`" class="no-underline width-640px">
        <div :class="`padding-05 flex items-center rounded-sm ${props.answerStatus === 'failed' ? 'bg-custom-negative' : 'bg-custom-positive'}`">
            <img
                :src="props.track.artwork_url" class="h-14 w-14 "
                :alt="props.track.title"/>
            <div class="flex flex-col margin-horizontal-075 text-white">
                <p class="">{{props.track.game}}</p>
                <p class="text-sm ">{{ props.track.title }}</p>
            </div>
            ...
            <div>
                <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none"
                    ...
                    <path class="text-white" d="M9 18l6-6-6-6"></path>
                </svg>
            </div>
        </div>
    </a>
...
<script setup>
const props = defineProps(['track','answerStatus']);
console.log('in SoundCloudWidget - track:', props.track);
